...
class OneCallDaoTest : NBOneCallDaoTest<OneCallDao, OneCallModelLocal>() {
    ...
    private lateinit var minutelyForecastDao: MinutelyForecastDao
    private lateinit var nationalWeatherAlertDao: NationalWeatherAlertDao

    override fun initiateDao(): OneCallDao = db.oneCallDao()
    ...
    override fun insert(entity: OneCallModelLocal) {
        dao.insertOneCall(entity.metadata)
    ...
    override fun createAct(metadataId: Long): Flow<OneCallModelLocal?> =
        dao.getOneCall(getLatLongFromMetadataId(metadataId), getLatLongFromMetadataId(metadataId))
